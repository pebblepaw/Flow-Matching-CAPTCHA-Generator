...
# ============================================================================
# MODEL-RELATED CONFIGURATION (affects tokenization output)
# ============================================================================
...
def preprocess_image(
    ...
    if method == "color_voting":
        result = color_voting_remove_black_hairline(image)
    elif method == "inpainting":
        result = inpainting_remove_black_hairline(image, inpaint_radius=inpaint_radius)
    elif method == "combined":
        # Color voting first, then inpainting
        result = color_voting_remove_black_hairline(image)
        result = inpainting_remove_black_hairline(result, inpaint_radius=inpaint_radius)
    else:
...
def visualize_pipeline(
    ...
    for idx, img_path in enumerate(images):
        ...
        if preprocessing_method == 'combined':
            # Combined method: color voting -> inpaint
            color_voted = color_voting_remove_black_hairline(img_rgb)
            preprocessed = inpainting_remove_black_hairline(color_voted, inpaint_radius=inpaint_radius)

            print(f"  [OK] Preprocessing (combined): Color voting + Inpainting(radius={inpaint_radius}) complete")
        else:
            color_voted = None
